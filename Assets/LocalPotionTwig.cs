...
public class LocalPotionTwig : MonoBehaviour
{
    ...
    [Header("State (local)")]
    public bool HasPotion; // inspector-visible local state
    ...
    public void FinishDip()
    {
        ...
        HasPotion = true;
        Log("HasPotion = TRUE (local).");
        ApplyChargedVfx(true);
    }

    private void Update()
    {
        // VFX sync (replaces Render())
        if (!_vfxInit)
        {
            _lastVfxHasPotion = HasPotion;
            _vfxInit = true;
        ...
        else if (HasPotion != _lastVfxHasPotion)
        {
            _lastVfxHasPotion = HasPotion;
            ApplyChargedVfx(HasPotion);
        }
    ...
    private void Log(string msg)
    {
        ...
        Debug.Log($"[LocalPotionTwig:{name}] {msg}", this);
    }
}
