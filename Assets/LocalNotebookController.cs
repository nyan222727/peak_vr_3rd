...
public class LocalNotebookController : MonoBehaviour
{
    ...
    #region Trigger with local candle (child trigger, parent tag = "Candle")

    private void OnTriggerEnter(Collider other)
    {
        if (_sequenceStarted || _sequenceFinished) return;

        // trigger collider is on candle child, tag is on parent
        var candleRoot = other.GetComponentInParent<Transform>();
        if (candleRoot == null || !candleRoot.CompareTag("Candle"))
            return;

        // Optional: if you add LocalCandle.cs (below), respect its lit state.
        var candle = candleRoot.GetComponent<LocalCandle>();
        if (candle != null && !candle.IsLit)
        return;

        Debug.Log($"Touched candle trigger: {other.name} (root: {candleRoot.name}). Start burning.");
        StartBurnSequence();
    }
