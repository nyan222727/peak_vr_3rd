...
public class GhostGrabInterference : NetworkBehaviour
{
    ...
    [Tooltip("Set this to a CHILD transform (mesh root). Do NOT shake the NetworkObject root if it has network transform.")]
    ...
    // Key: replicated “held” state so ANY client can find the held object.
    ...
    private double _lastHitSimTime = -999;
    ...
    public override void Spawned()
    {
        ...
        VLog($"Spawned. HasStateAuthority={Object?.HasStateAuthority} HasInputAuthority={Object?.HasInputAuthority}");
    }

    private void FixedUpdate()
    {
        // Only state authority is allowed to publish IsHeld (so everyone agrees).
        ...
        if (heldNow != _lastHeldLocal)
        {
            ...
            IsHeld = heldNow;
            VLog($"IsHeld -> {heldNow} (rb.isKinematic={_rb.isKinematic})");
        }
    }
    ...
    {
        if (verboseLogs)
            Debug.Log($"[GhostGrabInterference] RegisterGhostHit send -> {name} | grabbed={IsGrabbedNow} | stateAuth={Object?.StateAuthority}");

        RPC_RegisterGhostHit(playerPos, playerForward);
    }

    [Rpc(RpcSources.All, RpcTargets.StateAuthority)]
    private void RPC_RegisterGhostHit(Vector3 playerPos, Vector3 playerForward)
    {
        
        bool held = IsHeld || IsProbablyGrabbed(); // safety: allow either

        VLog($"RPC_RegisterGhostHit RECEIVED. held={held} IsHeld(net)={IsHeld} " +
             $"HasStateAuthority={Object?.HasStateAuthority} Tick={(Runner != null ? Runner.Tick : -1)}");

        if (!held)
        {
            VLog("RPC_RegisterGhostHit: ignored because object not held.");
            return;
        }

        double now = Runner != null ? Runner.SimulationTime : Time.timeAsDouble;

        if (now - _lastHitSimTime > comboWindowSeconds)
            _comboHits = 0;

        _comboHits++;
        _lastHitSimTime = now;

        float charge01 = Mathf.Clamp01((float)_comboHits / hitsToTriggerFinal);

        int seed = (Runner != null ? Runner.Tick : Time.frameCount) ^ (_comboHits * 997);
        RPC_PlayShake(charge01, seed);
    ...
    private void RPC_PlayShake(float charge01, int seed)
    {
    ...
    private void VLog(string msg)
    {
        ...
        Debug.Log($"[GhostGrabInterference:{name}] {msg}");
    }
}
